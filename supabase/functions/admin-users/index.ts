    ...
    if (req.method === 'POST') {
      ...
      if (action === 'update_user' || action === 'remove_user') {
        ...
        if (!isSuperAdmin) {
          ...
          if (
            !ut ||
            (ut.role !== 'admin' && ut.role !== 'admin_tenant' && ut.role !== 'compliance_officer')
          ) {
            throw new Error(
              'Acesso negado. Apenas administradores ou Compliance Officers do tenant podem gerenciar acessos.',
            )
          }
        }
