  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'update_user' || action === 'remove_user') {
        if (!target_user_id || !target_tenant_id) throw new Error('Parâmetros inválidos. Informe o usuário e a organização.')
        
        // Granular Security Check: Only 'admin' role in the specific tenant can mutate users
        if (!isSuperAdmin) {
          ...
          if (!ut || ut.role !== 'admin') {
             throw new Error('Acesso negado. Apenas administradores do tenant podem gerenciar acessos.')
          }
        }
    ...
    try {
      ...
      if (supabaseUrl && supabaseServiceKey) {
        ...
        await adminClient.from('system_errors').insert({
          error_message: error.message || 'Erro interno no admin-users',
          error_stack: error.stack || null,
          context: { function: 'admin-users' }
        })
      }
    } catch (logError) {
      console.warn("Failed to log to system_errors", logError)
    }
