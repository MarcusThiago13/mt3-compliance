...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Granular RBAC Check: Only 'admin' role in the specific tenant can send invites
    if (!isSuperAdmin) {
      ...
      if (
        ...
        throw new Error(
          'Acesso negado. Apenas administradores ou Compliance Officers do tenant podem processar convites.',
        )
      }
    }
    ...
    try {
      ...
      if (supabaseUrl && supabaseServiceKey) {
        ...
        await adminClient.from('system_errors').insert({
          error_message: error.message || 'Erro interno no admin-invite',
          error_stack: error.stack || null,
          context: { function: 'admin-invite' },
        })
      }
    } catch (logError) {
